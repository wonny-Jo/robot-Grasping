...
void BodyCalib::CalcMatrix(){
	...
	string file_path = "calib_mat20200813_RL_00_alignedxyz_0.00.txt";
	ofstream outFile(file_path.data());
	outFile << "Error : " << bestError << std::endl;

	for (int i = 0; i < RTMat.rows; i++) {
		for (int j = 0; j < RTMat.cols; j++) {
			outFile << RTMat.at<float>(i, j) << ", ";
		}
		outFile << std::endl;
	}
	outFile << "Average Error : %f mm\n" << bestError << std::endl;
	outFile << "Inlier Count : %d\n" << NumInlier << std::endl;
